...
/**
 * initData из запроса: заголовок ИЛИ query-параметр ?auth=.
 * Query нужен там, где заголовок поставить нельзя: <img>, <audio>,
 * ссылки на скачивание. Подпись и срок проверяются так же строго.
 */
export function initDataFromReq(req) {
  return initDataFromHeader(req) || (typeof req.query?.auth === 'string' ? req.query.auth : null);
}
