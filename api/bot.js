...
import {
  ...
  // тихие часы и режим доставки решаются в lib/digest.js
  firstSeenMedia,
  // приглашения (счётчик друзей; условием доступа не являются)
  userExists, addReferral, countReferrals,
} from '../lib/db.js';
import { queueNotice, pendingOwners, claimNotices, requeueNotices } from '../lib/db.js';
import {
  shouldQueue, noticeEnabled, isDigestDue, buildDigest, normalizeMode, msgWord,
} from '../lib/digest.js';
import {
  mediaTypeOf, mediaFileIdOf, mediaUniqueIdOf, forwardOriginDate,
  MEDIA_RU, fmtWhen, chatTitleOf,
...
export default async function handler(req, res) {
  ...
  // Попутный сброс очереди.
  //
  // Cron на Vercel в бесплатном тарифе запускается раз в сутки — для
  // почасовой сводки этого мало. Вебхук же вызывается постоянно, пока бот
  // используется, поэтому проверяем очередь заодно с обработкой апдейта.
  // Лимит маленький: вебхук обязан ответить Telegram быстро, иначе апдейт
  // будет доставлен повторно.
  try {
    await flushDigests({ limit: 5 });
  } catch (e) {
    console.warn('digest: попутный сброс не удался —', e?.message);
  }
...
async function onCommand(msg) {
  ...
  if (cmd === '/help') {
    await tg('sendMessage', {
      ...
      text:
        ...
        '📄 /privacy — политика конфиденциальности и пользовательское соглашение.\n\n' +
        '⚠️ Помните: в архив попадают сообщения ваших собеседников, и Telegram ' +
        'их об этом не уведомляет. Уведомить их — ваша обязанность по соглашению.',
    });
    return;
  }
  ...
  if (cmd === '/privacy' || cmd === '/terms') {
    // Адрес берём из переменной окружения: домен у превью и прода разный,
    // а зашитая ссылка молча ведёт не туда.
    ...
    await tg('sendMessage', {
      ...
      text:
        ...
        (base
          ...
          : 'Оба документа открываются в приложении: «Настройки» → ' +
            '«Конфиденциальность».\n\n') +
        'Коротко: файлы у нас не хранятся, только ссылки на них; архив ' +
        'удаляется целиком в один клик; данные никому не передаются.\n\n' +
        '⚠️ В архив попадают сообщения ваших собеседников, и Telegram их ' +
        'об этом не уведомляет. Ответственность за это лежит на вас — ' +
        'см. раздел 5 соглашения.',
      disable_web_page_preview: true,
    });
    return;
  }
  ...
  if (isPrivate) {
    await tg('sendMessage', {
      chat_id: msg.chat.id,
      text: 'Команды: /start — начать, /invite — позвать друзей, ' +
        '/help — как это работает, /privacy — документы, /status — проверка связи.',
    });
  }
}
...
// ─── УВЕДОМЛЕНИЯ ───
//
// Единственная точка отправки. Раньше три места слали сообщения напрямую и
// каждое само решало про тихие часы; теперь решение принимается здесь.
//
// kind: 'deleted' | 'edited' | 'fake'
async function notify(ownerTgId, targetChatId, notice, settings) {
  if (!targetChatId) return;
  if (!noticeEnabled(settings, notice.kind)) return;

  if (shouldQueue(settings)) {
    await queueNotice({ ownerTgId, targetChatId, ...notice });
    return;
  }
  await tg('sendMessage', { chat_id: Number(targetChatId), text: notice.text });
}

/**
 * Отправить всё, чему пришёл срок.
 *
 * Вызывается из cron и попутно из вебхука. Двойной вызов безопасен: события
 * забираются из очереди атомарно, и второй вызов просто не найдёт ничего.
 *
 * @param limit сколько владельцев обработать за раз — serverless-функция
 *              ограничена по времени, и лучше разнести работу по вызовам,
 *              чем упереться в таймаут на середине.
 */
export async function flushDigests({ limit = 40, now = new Date() } = {}) {
  const owners = await db(() => pendingOwners(limit));
  if (!owners?.length) return { checked: 0, sent: 0 };

  let sent = 0;
  for (const o of owners) {
    if (!isDigestDue(o.settings, o.oldestAt, now)) continue;

    const items = await db(() => claimNotices(o.tgId));
    if (!items?.length) continue;   // забрал другой вызов — это норма

    // Адресат один и тот же у всех событий владельца; на всякий случай
    // берём из последнего — если человек переподключал бота, он свежее.
    const target = items[items.length - 1].targetChatId;
    const text = buildDigest(items, { mode: normalizeMode(o.settings.notifyMode) });
    if (!text) continue;

    const r = await tg('sendMessage', { chat_id: Number(target), text });

    // Не доставили (бот заблокирован, лимит) — возвращаем в очередь, иначе
    // события пропадут навсегда. Предохранитель STALE_HOURS не даст им
    // копиться вечно, если человек заблокировал бота насовсем.
    if (r?.ok) sent += 1;
    else await db(() => requeueNotices(o.tgId, items));
  }
...
async function notifyFake(owner, msg, mediaType, firstSeen, origDate) {
  ...
  // Текст собран здесь: при отправке из очереди деталей уже не будет.
  await notify(owner, bc.user_chat_id, { kind: 'fake', chatTitle: who, text }, s);
}
...
async function onBusinessEdit(msg) {
  await db(async () => {
    ...
    if (bc?.user_chat_id) {
      const s = await getUserSettings(owner);
      const who = chatTitleOf(msg.chat);
      await notify(owner, bc.user_chat_id, {
        kind: 'edited',
        chatTitle: who,
    ...
    if (n > 0) {
      ...
      if (bc?.user_chat_id) {
        ...
        await notify(owner, bc.user_chat_id, {
          ...
          chatTitle: who,
          count: n,
          text: `🗑 В чате «${who}» удалили ${n} ${msgWord(n)}. ` +
