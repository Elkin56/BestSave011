...
async function loadSettings() {
  ...
  catch { S.settings = { notifyDeleted: true, notifyEdited: false, notifyFake: true, notifyMode: 'instant', offline: true }; render(); }
}

// Выбор режима доставки. Отдельно от toggleSetting: там переключение
// булева значения, здесь — выбор одного из четырёх.
async function setMode(mode) {
  if (!S.settings || S.settings.notifyMode === mode) return;
  S.settings.notifyMode = mode;
  render();
...
function Privacy() {
  ...
  return `
    ...
    <div class="card" style="margin:0 16px;overflow:hidden">
      <a class="row" href="/privacy.html" target="_blank" rel="noopener">
        <span class="chip" style="width:32px;height:32px;border-radius:11px;background:var(--g-purple)">${sv('doc',15,'#fff',2.3)}</span>
        <span class="row-main"><span class="row-title">Политика конфиденциальности</span>
        <span class="row-sub">Какие данные, где хранятся и как их удалить</span></span>
        ${sv('arrow',13,'var(--txt-lo)')}
      </a>
      <div class="div"></div>
      <a class="row" href="/terms.html" target="_blank" rel="noopener">
        <span class="chip" style="width:32px;height:32px;border-radius:11px;background:var(--g-orange)">${sv('shield',15,'#fff',2.3)}</span>
        <span class="row-main"><span class="row-title">Пользовательское соглашение</span>
        <span class="row-sub">Что можно, что запрещено и за что отвечаете вы</span></span>
        ${sv('arrow',13,'var(--txt-lo)')}
      </a>
    </div>
...
function Settings() {
  ...
  // Режим доставки: одна выбранная строка, остальные — варианты.
  const mode = s.notifyMode || 'instant';
  const modes = [
    ['instant','Сразу','Каждое событие — отдельным сообщением'],
    ['hourly','Раз в час','Накопленное за час придёт одной сводкой'],
    ['daily','Раз в день','Одна сводка утром, в 9:00 по вашему времени'],
    ['silent','Только в приложении','Бот не пишет вообще — события смотрите здесь'],
  ];
  const modeRow = ([key, title, sub]) => `
    <button class="row" data-mode="${key}" style="width:100%;text-align:left">
      <span class="chip" style="width:26px;height:26px;border-radius:9px;
        background:${mode === key ? 'var(--g-green)' : 'var(--surf-hi)'}">
        ${mode === key ? sv('check',13,'#04231A',3) : ''}</span>
      <span class="row-main">
        <span class="row-title">${title}</span>
        <span class="row-sub">${sub}</span>
      </span>
  ...
  return `
    <div class="sec">Как доставлять</div>
    <div class="card" style="margin:0 16px;overflow:hidden">
      ${modes.map((m,i) => `${i ? '<div class="div"></div>' : ''}${modeRow(m)}`).join('')}
    </div>
    ${s.pending ? `<div class="note" style="margin:10px 16px 0;color:var(--gold)">
      ${s.pending} ${s.pending === 1 ? 'событие ждёт' : s.pending < 5 ? 'события ждут' : 'событий ждут'}
      ближайшей сводки.</div>` : ''}

    <div class="sec">О чём сообщать</div>
    <div class="card" style="margin:0 16px;overflow:hidden${mode === 'silent' ? ';opacity:.5' : ''}">
    ...
    <div class="note" style="margin:10px 16px 0">
      ...
      Ночные события не теряются: они копятся и приходят одной сводкой,
      когда тишина закончится.
    </div>
