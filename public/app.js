...
async function loadSettings() {
  ...
  catch { S.settings = { notifyDeleted: true, notifyEdited: false, notifyFake: true, saveOnReply: true, offline: true }; render(); }
}
...
function Settings() {
  ...
  return `
    ...
    <div class="sec">Сохранение по ответу</div>
    <div class="card" style="margin:0 16px;overflow:hidden">
      ${row('saveOnReply','Ответ = сохранить','Ответьте на файл в подключённом чате — хоть точкой — и его копия придёт вам в личку с ботом', s.saveOnReply !== false)}
    </div>
    <div class="note" style="margin:10px 16px 0">
      Работает для фото, видео, голосовых, кружков, GIF и документов.
      Копия приходит один раз, сколько бы ответов ни было.
    </div>
    <div class="note" style="margin:10px 16px 0">
