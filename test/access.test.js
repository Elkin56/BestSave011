...
describe('уведомления уважают тихие часы', () => {
  ...
  test('все три уведомления идут через единую точку отправки', () => {
    // Раньше удаление, изменение и фейк-контроль сами проверяли тишину и
    // при её наступлении просто ничего не отправляли — уведомление молча
    // терялось. Теперь решение принимает notify(): ночью событие встаёт
    // в очередь и приходит утром сводкой.
    const calls = bot.match(/await notify\(/g) || [];
    assert.ok(calls.length >= 3, `ожидалось 3+ вызовов notify, найдено ${calls.length}`);

    // Прямых sendMessage в обход notify быть не должно — иначе часть
    // событий снова начнёт будить ночью.
    assert.doesNotMatch(bot, /isQuietNow\(s\)/,
      'проверка тишины переехала в lib/digest.js');
  });

  test('ночные события не теряются, а копятся', () => {
    assert.match(bot, /shouldQueue/);
    assert.match(bot, /queueNotice/);
  });
});
