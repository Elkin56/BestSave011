...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.status(200).json({
      ...
      // Признак владельца: по нему фронтенд решает, показывать ли вкладку
      // админки. Сам эндпоинт /api/admin проверяет права независимо —
      // подделка этого флага на клиенте ничего не даёт.
      isAdmin: (process.env.ADMIN_TG_IDS || '').split(',')
        .map((x) => x.trim()).filter(Boolean).includes(String(u.id)),
      plan: {
