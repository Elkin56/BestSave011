...
export default async function handler(req, res) {
  ...
  try {
    ...
    // В выгрузку попадают ТОЛЬКО видимые пользователю копии — иначе
    // скачанный файл содержал бы чужую переписку из общего чата.
    const { rows: msgs } = await q(
      `SELECT m.id, m.sender_name, m.text, m.media_type, m.media_file_id,
       ...
       WHERE m.chat_id = $2 AND ${VISIBLE}
       ORDER BY m.id DESC LIMIT $3`,
      [req.tgUser.id, internalId, MAX_MESSAGES]
    );
