...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Список пользователей. Намеренно БЕЗ содержимого: имя, username,
    // когда подключился, сколько чатов и сообщений, живо ли Business.
    // Названий чатов здесь нет — они выдали бы, с кем человек переписывается.
    // Текстов и медиа нет тем более. Ограничение LIMIT: список для обзора,
    // а не выгрузка всей базы.
    const list = await q(
      `SELECT u.tg_id, u.first_name, u.username, u.is_premium, u.created_at,
              COUNT(DISTINCT cl.chat_id)::int AS chats,
              COUNT(m.id)::int AS messages,
              MAX(m.created_at) AS last_msg,
              bool_or(bc.is_enabled) AS business
       FROM app_user u
       LEFT JOIN chat_link cl ON cl.user_id = u.id
       LEFT JOIN message m ON m.owner_tg_id = u.tg_id
       LEFT JOIN business_connection bc ON bc.user_tg_id = u.tg_id
       GROUP BY u.id, u.tg_id, u.first_name, u.username, u.is_premium, u.created_at
