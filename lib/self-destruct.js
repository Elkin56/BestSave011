...
//
// ВАЖНО (проверено по официальной документации core.telegram.org/bots/api,
// июль 2026): поля ttl_seconds НЕТ ни в Message, ни в PhotoSize/Video/Voice/
// VideoNote — оно существует только в MTProto-конструкторах для клиентов,
// которые ГОТОВЯТ отправку такого фото, а не в апдейтах, которые получает
// бот. Иными словами, isSelfDestructing()/getTTL() ниже физически не могут
// сработать на реальном входящем сообщении — бот не в состоянии узнать,
// стоит ли у файла таймер. Поэтому в api/bot.js эти функции больше не
// используются: вместо выборочного детекта там теперь безусловная
// подстраховка (перезаливка) для ЛЮБОГО фото/видео/голосового/кружка
// из бизнес-чата — см. rearchiveVanishableMedia() в api/bot.js.
// Модуль оставлен для обратной совместимости со scripts/check-self-destruct.js
// и как честная документация того, что было исследовано.
