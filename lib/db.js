...
export async function ensureSchema() {
  await q(`
    ...
    -- Удаление чужих данных не должно быть бесследным: здесь остаётся, кто,
    -- когда, у кого и сколько удалил. Записи не удаляются вместе с
    -- пользователем — иначе журнал терял бы смысл ровно в тот момент,
    -- когда он нужен.
    ...
    CREATE INDEX IF NOT EXISTS idx_admin_action_at ON admin_action (at DESC);
    ...
    -- число приглашённых ни на что не влияют. Таблица осталась ради счётчика
    -- в /invite. В базах прошлых версий сохраняется колонка gate_passed_at —
    -- её никто не читает, дропать не станем: удалять чужие данные молча нельзя.
    --
...
/**
 * Удалить архив ОДНОГО чата у одного пользователя.
 *
 * Удаляются только копии этого владельца: в групповом чате у других
 * участников свои копии тех же сообщений, и они остаются нетронутыми.
 *
 * @param unlink отвязать ли чат совсем. false — чат остаётся подключённым
 *               и продолжит наполняться заново.
 */
export async function eraseChatForUser(tgId, tgChatId, { unlink = false } = {}) {
  const { rows } = await q(
    `SELECT c.id, cl.user_id
     FROM chat c
     JOIN chat_link cl ON cl.chat_id = c.id
     JOIN app_user u ON u.id = cl.user_id
     WHERE c.tg_chat_id = $1 AND u.tg_id = $2`,
    [tgChatId, tgId]
  );
  // Чат не найден или не принадлежит этому человеку — считаем, что удалять
  // нечего. Отдельный ответ «чужой чат» подтвердил бы его существование.
  if (!rows.length) return null;

  const chatId = rows[0].id;
  const msgs = await q(
    `DELETE FROM message WHERE chat_id = $1 AND owner_tg_id = $2`,
    [chatId, tgId]
  );

  let unlinked = false;
  if (unlink) {
    await q(`DELETE FROM chat_link WHERE chat_id = $1 AND user_id = $2`, [chatId, rows[0].user_id]);
    // Чат без единого владельца больше никому не нужен.
    await q(
      `DELETE FROM chat c WHERE c.id = $1
       AND NOT EXISTS (SELECT 1 FROM chat_link cl WHERE cl.chat_id = c.id)`,
      [chatId]
    );
    unlinked = true;
  }

  return { messages: msgs.rowCount || 0, unlinked };
}

/**
 * Очистка со стороны владельца продукта.
 *
 * mode 'archive' — стереть переписку, оставить учётную запись и подключения:
 *   бот продолжит работать, архив начнётся заново.
 * mode 'full' — удалить пользователя целиком (то же, что делает он сам).
 *
 * Названия чатов наружу не отдаются: панель владельца намеренно не показывает,
 * с кем человек переписывается.
 */
export async function adminEraseUser(targetTgId, mode = 'archive') {
  const { rows } = await q('SELECT id FROM app_user WHERE tg_id = $1', [targetTgId]);
  if (!rows.length) return null;

  if (mode === 'full') return { mode, ...(await eraseUserData(targetTgId)) };

  const msgs = await q('DELETE FROM message WHERE owner_tg_id = $1', [targetTgId]);
  await q('DELETE FROM pending_notice WHERE owner_tg_id = $1', [targetTgId]);
  return { mode: 'archive', messages: msgs.rowCount || 0, chats: 0, connections: 0 };
}

// ─── Обслуживание хранилища ───
//
// Категории мусора. Каждая — данные, которые уже никому не принадлежат:
// удалять их не значит лишать кого-то архива.
//
// Намеренно НЕ включено автоматическое удаление «старых» сообщений: политика
// конфиденциальности обещает, что архив живёт, пока владелец сам его не
// удалит. Срок хранения ввести можно, но это правка политики, а не уборка.
const SWEEP = {
  orphanChats: {
    label: 'Чаты без владельцев',
    count: `SELECT COUNT(*)::int AS n FROM chat c
            WHERE NOT EXISTS (SELECT 1 FROM chat_link cl WHERE cl.chat_id = c.id)`,
    del: `DELETE FROM chat c
          WHERE NOT EXISTS (SELECT 1 FROM chat_link cl WHERE cl.chat_id = c.id)`,
  },
  orphanMessages: {
    label: 'Сообщения удалённых пользователей',
    count: `SELECT COUNT(*)::int AS n FROM message m
            WHERE m.owner_tg_id IS NOT NULL
              AND NOT EXISTS (SELECT 1 FROM app_user u WHERE u.tg_id = m.owner_tg_id)`,
    del: `DELETE FROM message m
          WHERE m.owner_tg_id IS NOT NULL
            AND NOT EXISTS (SELECT 1 FROM app_user u WHERE u.tg_id = m.owner_tg_id)`,
  },
  staleNotices: {
    label: 'Недоставленные уведомления старше 30 дней',
    count: `SELECT COUNT(*)::int AS n FROM pending_notice
            WHERE created_at < now() - interval '30 days'`,
    del: `DELETE FROM pending_notice WHERE created_at < now() - interval '30 days'`,
  },
  deadConnections: {
    label: 'Отключённые бизнес-подключения',
    count: `SELECT COUNT(*)::int AS n FROM business_connection bc
            WHERE bc.is_enabled = false
              AND NOT EXISTS (SELECT 1 FROM app_user u WHERE u.tg_id = bc.user_tg_id)`,
    del: `DELETE FROM business_connection bc
          WHERE bc.is_enabled = false
            AND NOT EXISTS (SELECT 1 FROM app_user u WHERE u.tg_id = bc.user_tg_id)`,
  },
};

export const SWEEP_KEYS = Object.keys(SWEEP);

/** Что можно вычистить — без изменений в базе. */
export async function storageAudit() {
  const items = [];
  for (const [key, spec] of Object.entries(SWEEP)) {
    const { rows } = await q(spec.count);
    items.push({ key, label: spec.label, n: rows[0]?.n || 0 });
  }
  const { rows: size } = await q(`SELECT pg_database_size(current_database())::bigint AS b`);
  return { items, dbBytes: Number(size[0]?.b || 0) };
}

/** Выполнить уборку по выбранным категориям. Возвращает удалённое по каждой. */
export async function storageSweep(keys) {
  const chosen = (Array.isArray(keys) ? keys : []).filter((k) => SWEEP[k]);
  const done = [];
  // Порядок важен: сообщения осиротевших чатов уходят каскадом при удалении
  // чата, поэтому чаты чистим первыми — иначе второй запрос считал бы то,
  // чего уже нет.
  for (const key of SWEEP_KEYS.filter((k) => chosen.includes(k))) {
    const r = await q(SWEEP[key].del);
    done.push({ key, label: SWEEP[key].label, deleted: r.rowCount || 0 });
  }
  return done;
}
...
export async function logAdminAction(adminTgId, action, targetTgId, details) {
  await q(
    `INSERT INTO admin_action (admin_tg_id, action, target_tg_id, details)
     VALUES ($1, $2, $3, $4)`,
    [adminTgId, action, targetTgId || null, details ? String(details).slice(0, 500) : null]
  );
}

export async function recentAdminActions(limit = 20) {
  const { rows } = await q(
    `SELECT admin_tg_id, action, target_tg_id, details, at
     FROM admin_action ORDER BY at DESC LIMIT $1`,
    [limit]
  );
  return rows.map((r) => ({
    admin: String(r.admin_tg_id),
    action: r.action,
    target: r.target_tg_id ? String(r.target_tg_id) : null,
    details: r.details,
    at: r.at,
  }));
}
